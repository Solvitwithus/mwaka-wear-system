...
const ApproveExitPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filter for pending exit requests
        const pendingRequests = exitData.filter(
          (req: ExitRequest) => req.status !== "approved"
        );
        console.log("Pending requests:", pendingRequests);

        setExitRequests(pendingRequests);
        ...
        setError("Failed to fetch exit requests or approvers.");
      }
    };
  ...
  const handleApproveExit = async (entry: ExitRequest) => {
    try {
      ...
      const payload = {
        ...
        reasonToExit: entry.reasonToExit,
        exitDate: entry.exitDate,
        exitInterview: entry.exitInterview,
        knowledgeTransferPlan: entry.knowledgeTransferPlan,
      };
      ...
      setError("Failed to approve exit request.");
    }
  };

  return (
    <div className="bg-[#EFEFEF] m-1 rounded-md p-1 h-fit">
      <h4 className="text-black font-medium text-base ml-1">Approve Exit Requests</h4>
      {error && (
                  ...
                  <tr key={entry.id} className="border-t">
                    ...
                    <td className="p-1 border">
                      <button
                        ...
                        onClick={() => handleApproveExit(entry)}
                      >
...
export default ApproveExitPage;