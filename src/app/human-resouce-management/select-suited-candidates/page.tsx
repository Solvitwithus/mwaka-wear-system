...
const Page = () => {
  ...
  useEffect(() => {
    const fetchCandidates = async () => {
      try {
        ...
        const appliedCandidates = data.filter((candidate: Candidate) => candidate.status === "Shortlisted");
  ...
  const handleLocalDataParsing = async (entry: Candidate) => {
    try {
      ...
      setCandidates((prev) =>
        prev.map((item) =>
          item.id === entry.id ? { ...item, status: "suited" } : item
        )
      );
  ...
  return (
    <div className="bg-[#EFEFEF] m-1 rounded-md p-1 h-fit">
      <h4 className="text-black font-medium text-base ml-1">Conduct Interview</h4>
