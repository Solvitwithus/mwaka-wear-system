...
import {LucidePanelLeftOpen} from 'lucide-react'
// --- Types ---
type Candidate = {
  id: string;
...
const Page = () => {
  ...
  useEffect(() => {
    const fetchCandidates = async () => {
      try {
        ...
        const appliedCandidates = data.filter((candidate: Candidate) => candidate.status === "Oriented");
  ...
  const handleLocalDataParsing = async (entry: Candidate) => {
    try {
      ...
      router.push("/human-resouce-management/issuarance-of-working-tools/tools")
    } catch (error) {
  ...
  return (
    <div className="bg-[#EFEFEF] m-1 rounded-md p-1 h-fit">
      <h4 className="text-black font-medium text-base ml-1">Issue Working tools to employee</h4>
                ...
                paginatedCandidates.map((entry, index) => (
                  <tr key={entry.id} className="border-t">
                    ...
                    <td className="p-2 border">
                      ...
                      >
                        Issue Tools<LucidePanelLeftOpen/>
                      </button>
                    </td>
                  </tr>
                ))
