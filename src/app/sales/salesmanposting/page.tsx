...
const [salePerson, setsalePerson] = useState<salePerson[]>([])
const [onAir, setonAir] = useState<air>({
  salesPersonCode:""
})
  // added
 ...
 const fetchItem = useCallback(async()=>{
     try {
      const [itmRes,vehicleRes,driverRes,shiftRes,branchres,clientRes,salespersonRes] = await Promise.all([
...
axios.get("/api/auth/salesperson")
      ])
...
setsalePerson(salespersonRes.data)
    } catch (error) {
...
const handleQuotqtionAdd = async (e: FormEvent) => {
  ...
  try {
    const res = await axios.post("/api/auth/salesman-entry", payload);
    ...
    return(
        ...
        <div className='bg-[#EFEFEF] m-1 rounded-md p-1 h-fit'> <form onSubmit={handleQuotqtionAdd}>
            <h4 className='text-black font-medium ml-1 text-sm '>New Sales Order Entry</h4>
            {/* upper section */}
...
<select 
  ...
  value={onAir.salesPersonCode} 
  onChange={handleSalesPersonChange} 
  className="my-[1px] w-52 bg-[#f5c5c5]" 
  required
>
  <option value="">Select Sales Person</option>
  {salePerson.map((val, idx) => (
    <option key={idx} value={val.employeeCode}>
      {val.firstName}: {val.employeeCode}
    </option>
  ))}
</select>
</div>
            ...
            {/* Table Section */}
           <div className='border-[1px] border-black mx-1 rounded-md mb-2'>
  <h6 className='px-2 py-1 font-semibold text-sm text-black'>Populate Entry</h6>
...
<button type='button'
    ...
    {/* lower section */}
            <div className='border-[1px] border-black mx-1 rounded-md mb-2'>
<h6 className='text-center text-sm text-black font-semibold'>Sales Entry Delivery Details</h6>
{/* innerbody container */}
