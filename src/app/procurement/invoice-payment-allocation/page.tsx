...
const Page = () => {
  ...
  const [parsedData, setParsedData] = useState<PurchaseReqEntry | null>(null);
  const [partialAllocations, setPartialAllocations] = useState<{ [key: string]: number }>({});
  const [paymentData, setPaymentData] = useState<{ [key: string]: PaymentTracking }>({});
  ...
  // --- Fetch Payment Data for Selected Entry ---
  useEffect(() => {
    if (parsedData) {
      const fetchPaymentData = async () => {
        try {
          const res = await axios.get(`/api/auth/supplier-payment-tracking?purchaseEntryId=${parsedData.id}`);
          const payments = res.data.data || [];
          const paymentMap = payments.reduce((acc: { [key: string]: PaymentTracking }, payment: PaymentTracking) => {
            acc[payment.itemId] = payment;
            return acc;
          }, {});
  ...
  const handleAllocateAll = async (entry: PurchaseReqEntry) => {
    try {
      for (const item of entry.items) {
        if (item.itemId && item.total) {
          await axios.post("/api/auth/supplier-payment-tracking", {
      ...
      const res = await axios.get(`/api/auth/supplier-payment-tracking?purchaseEntryId=${entry.id}`);
      const payments = res.data.data || [];
      const paymentMap = payments.reduce((acc: { [key: string]: PaymentTracking }, payment: PaymentTracking) => {
        acc[payment.itemId] = payment;
        return acc;
      ...
      alert("Failed to allocate all.");
    }
  };

  const handlePartialAllocation = async (entry: PurchaseReqEntry) => {
    try {
      for (const item of entry.items) {
        if (item.itemId && partialAllocations[item.itemId]) {
          await axios.patch("/api/auth/prepayment", {
            itemId: item.itemId,
            partialAllocation: partialAllocations[item.itemId],
          });
        }
      }
      alert("Partial allocation submitted.");
      setPartialAllocations({}); // Reset allocations after submission
      // Refresh payment data
      const res = await axios.get(`/api/auth/supplier-payment-tracking?purchaseEntryId=${entry.id}`);
      const payments = res.data.data || [];
      const paymentMap = payments.reduce((acc: { [key: string]: PaymentTracking }, payment: PaymentTracking) => {
      ...
      alert("Failed to submit partial allocation.");
    }
  };

  const handleLocalDataParsing = (entry: PurchaseReqEntry) => {
    setParsedData(entry);
    setPartialAllocations({}); // Reset partial allocations when opening modal
  };

  const renderParsedData = (entry: PurchaseReqEntry) => {
        ...
        <table className="w-full text-sm border-collapse border border-white">
          <thead>
            <tr className="bg-gray-800">
              ...
              <th
                className="text-green-500 text-xs cursor-pointer p-2"
              ...
              <th className="p-2">Prev Allocation</th>
          ...
          <tbody>
            {entry.items.map((item, idx) => (
              <tr key={idx} className="border-t border-white">
                <td className="p-2">{entry.purchaseAdditionalInfo?.SupplierReference ?? "N/A"}</td>
                <td className="p-2">{item.itemId ?? "N/A"}</td>
                <td className="p-2">{item.itemName}</td>
                <td className="p-2">{item.quantity}</td>
                <td className="p-2">{entry.supplier?.name ?? "N/A"}</td>
                <td className="p-2">
                  {entry.purchaseAdditionalInfo?.reqDate
                    ? new Date(entry.purchaseAdditionalInfo.reqDate).toLocaleDateString()
                    : "-"}
                </td>
                <td className="p-2">{item.total ?? "N/A"}</td>
                <td className="p-2">{entry.shipping}</td>
                <td className="p-2">
                  <input
                    type="number"
                ...
                <td className="p-2">
                  <input
                    ...
                    className="w-16 text-black"
                    disabled
                    value={paymentData[item.itemId || ""]?.allocatedAmount ?? 0}
                  />
                </td>
                <td className="p-2">
                  <input
                    ...
                    className="w-16 text-black"
                    disabled
                    value={paymentData[item.itemId || ""]?.balance ?? (item.total ?? 0)}
                  />
                </td>
              </tr>
            ))}
  ...
  return (
    <div className="bg-[#EFEFEF] m-1 rounded-md p-1 h-fit">
      <h4 className="text-black font-medium text-base ml-1">Allocate Payment to invoices</h4>
      ...
      {/* Parsed Data Modal */}
      {parsedData && renderParsedData(parsedData)}
